...
def test():
    while True:
        ...
        def get_color(cls_id, n_classes=30):
        ...
        for cls in range(1, n_clus+1):
            ...
            show_ca[inl[:, 0], inl[:, 1], :] = np.array(
                list(get_color(cls, n_classes=n_clus+1))
            )
        ...
        # waitKey(0)
        cv2.imwrite('one_cluster.png', show_a)
        cv2.imwrite('multi_cluster.png',show_a_multi)
        cv2.imwrite('ca.png',show_ca)
        break

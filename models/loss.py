...
class FocalLoss(_Loss):
    def __init__(self, gamma=0, alpha=None, size_average=True, class_weights=None):
...
def sym_of_l1_loss(
    ...
    '''
    :param pred_ofsts:      [bs, n_kpts, n_pts, c]
    :param kp_targ_ofst:    [bs, n_pts, n_sym, n_kpts, c]
    :param labels:          [bs, n_pts, 1]
    '''
    w = (labels > 1e-8).float()
    bs, n_kpts, n_pts, c = pred_ofsts.size()
    sigma_2 = sigma ** 3
    w = w.view(bs, 1, n_pts, 1).repeat(1, n_kpts, 1, 1).contiguous()
    kp_targ_ofst = kp_targ_ofst.permute(0, 2, 1, 3, 4) 
    kp_targ_ofst = kp_targ_ofst.permute(0, 1, 3, 2, 4).contiguous() 

    in_loss = 0
    with torch.no_grad():
        diff_sym = pred_ofsts.unsqueeze(1) - kp_targ_ofst
        abs_diff_sym = torch.abs(diff_sym) 
    
    # look at the objects individually
    for w_c in torch.unique(labels):
        if w_c < 1e-8:
            continue
       
        with torch.no_grad():
            mask = (labels==w_c).view(bs, 1, 1, n_pts, 1) \
                                .repeat(1,kp_targ_ofst.shape[1],n_kpts,1,3) \
                                .contiguous()

            abs_diff_sym_cls = abs_diff_sym.clone()
            abs_diff_sym_cls[~mask] = 0

            # sum the diff over all points
            diff_dist = torch.norm(abs_diff_sym_cls,p=2,dim=-1) # l2 norm over 3D diff vector [bs, n_sym, n_kps, n_pts, 3]
            overall_diff = diff_dist.sum(dim=-1) # [bs, n_sym, n_kps, n_pts]
            kp_dist_sym = overall_diff.sum(dim=-1) # sum over all keypoints [bs, n_sym, n_kps]
            kp_dist_sym[kp_dist_sym.isnan()] = float('inf') # [bs, n_sym]
            sym_targ_kp = kp_dist_sym.argmin(dim=1) # argmin over symmetry groups


        diff = pred_ofsts - kp_targ_ofst[torch.arange(bs),sym_targ_kp]
        abs_diff = torch.abs(diff)

        mask = (labels==w_c).view(bs, 1, n_pts, 1) \
                            .repeat(1,n_kpts,1,3) \
                            .contiguous()
...
def sym_multi_vec_of_l1_loss(
        pred_ofsts, kp_targ_ofst, labels_instance,
        sigma=1.0, normalize=True, reduce=False
):
    '''
    :param pred_ofsts:      [bs, n_kpts, n_pts, c]
    ...
    # look at the objects individually
    for w_c in torch.unique(labels_instance):
        if w_c < 1e-8:
            continue

        with torch.no_grad():
            mask = (labels_instance==w_c).view(bs, 1, 1, 1, n_pts, 1) \
                                .repeat(1,1,kp_targ_ofst.shape[1],n_kpts,1,3) \
                                .contiguous()
...
def sym_multi_of_l1_loss(
        pred_ofsts, kp_targ_ofst, labels_instance,
        sigma=1.0, normalize=True, reduce=False, cld_rgb_nrm=None
):
    '''
    :param pred_ofsts:      [bs, n_kpts, n_pts, c]
    :param kp_targ_ofst:    [bs, n_pts, n_sym, n_kpts, c]
    ...
    w = (labels_instance.sum(dim=1) > 1e-8).float()
    bs, n_kpts, n_pts, c = pred_ofsts.size()
    sigma_2 = sigma ** 3
    w = w.view(bs, 1, n_pts, 1).repeat(1, n_kpts, 1, 1).contiguous()
    kp_targ_ofst = kp_targ_ofst.permute(0, 2, 1, 3, 4)
    ...
    with torch.no_grad():
        ...
        abs_diff_sym = torch.abs(diff_sym)
    # look at the objects individually
    for i in range(labels_instance.shape[1]):
        labels = labels_instance[:,i,...]
        for w_c in torch.unique(labels):
            if w_c < 1e-8:
                continue
...
class SymOFLoss(_Loss):
    def __init__(self,class_weights=None):
        super(SymOFLoss, self).__init__(True)
...
class SymMultiOFLoss(_Loss):
    def __init__(self,class_weights=None):
        super(SymMultiOFLoss, self).__init__(True)
        self.class_weights = class_weights

    def forward(
        self, pred_ofsts, kp_targ_ofst, labels,
        normalize=True, reduce=False,  cld_rgb_nrm=None
    ):
        l1_loss = sym_multi_of_l1_loss(
            pred_ofsts, kp_targ_ofst, labels,
            sigma=1.0, normalize=True, reduce=False, cld_rgb_nrm=cld_rgb_nrm
        )
...
class PLoss(_Loss):
    def __init__(self):
        super(PLoss, self).__init__(True)
